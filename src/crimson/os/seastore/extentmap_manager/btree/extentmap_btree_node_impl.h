...
struct ExtMapInnerNode
  ...
  mutate_mapping_ret mutate_mapping(Transaction &transaction,
                                    uint32_t lo, mutate_func_t &&f) final;
...
// should make it common
  ...
  split_entry_ret split_entry(Transaction &t, uint32_t lo,
  ...
  merge_entry_ret merge_entry(Transaction &t, uint32_t lo,
              internal_iterator_t iter, ExtMapNodeRef entry);

  internal_iterator_t get_containing_child(uint32_t lo);
...
struct ExtMapLeafNode
 ...
 using internal_iterator_t = const_iterator;
  ...
  insert_ret insert(Transaction &transaction, uint32_t lo, lext_map_val_t val) final;
  ...
  get_leaf_entries(uint32_t lo, uint32_t len);
