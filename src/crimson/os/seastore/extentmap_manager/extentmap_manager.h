...
{
public:
  uint32_t logical_offset = 0;  //offset in object
  ...
  void dump(ceph::Formatter* f) const;
    ...
    uint32_t lextent_start() const {
      return logical_offset - lextent_offset;
    }
    ...
    uint32_t lextent_end() const {
      return (logical_end() + PAGE_SIZE -1)& ~(PAGE_SIZE -1) ;
    }
    // return true if any piece of the lextent is out of
    // the given range [o, o + l].
    bool lextent_escapes_range(uint32_t o, uint32_t l) const {
      return lextent_start() < o || lextent_end() > o + l;
    }
  ~Extent() {}
};
...
public:
  ...
  using lextent_ertr = TransactionManager::read_extent_ertr;
  using seek_lextent_ertr = lextent_ertr;
  ...
  virtual seek_lextent_ret
    seek_lextent(Transaction &t, uint32_t lo, uint32_t len) = 0;
  ...
  virtual add_lextent_ret 
    add_lextent(Transaction &t, uint32_t lo, lext_map_val_t val) = 0;
  ...
  virtual has_any_ret has_any_lextents(Transaction &t, uint32_t offset, uint32_t length) = 0;

  /// consolidate adjacent lextents in extent_map
  using compress_ertr = lextent_ertr;
  using compress_ret =  compress_ertr::future<int>;
  virtual compress_ret compress_extent_map(Transaction &t, uint32_t offset, uint32_t length) = 0;
