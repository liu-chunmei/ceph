...
enum class ordering_t : uint8_t{
  less = 0,
  equivalent = 1,
  greater = 2
};
  
static ordering_t compare_string(const std::string& x, const std::string& y){
 ...
 * K(fixed) -> V(string) with internal representations KINT.
 *
...
class StringKVInnerNodeLayout {
  ...
  using L = absl::container_internal::Layout<ceph_le32, MetaInt, KINT>;
...
public:
  ...
  struct iter_t {
    ...
    K get_node_key() const {
      KINT kint = node->get_node_key_ptr()[index];
      return K(kint);
    }
...
public:
  ...
  const_iterator string_upper_bound(std::string str) const {
    ...
    for (; ret != iter_end(); ++ret) {
     std::string s = ret->get_node_val();
      if (compare_string(s, str) == ordering_t::greater)
        break;
    }
      return ret;
  }
  ...
  const_iterator find_string_key(const std::string &str) const {
    ...
    for (; ret != iter_end(); ++ret) {
     std::string s = ret->get_node_val();
      if (compare_string(s, str) ==  ordering_t::equivalent)
        break;
    }
    return ret;
  }
  ...
  const_iterator get_split_pivot() const {
    uint32_t total_size = K(get_node_key_ptr()[get_size()-1]).key_off;
  ...
  uint32_t used_space() {
    ...
    if (count) {
      K last_key = K(get_node_key_ptr()[count-1]);
      return last_key.key_off + count * sizeof(KINT);
    } else {
      return 0;
    }
  }
  ...
  bool is_overflow(size_t size){
    return free_space() < (sizeof(KINT) + size);
  }
  bool under_median(){
  ...
  {
    uint32_t left_size = K(left.get_node_key_ptr()[left.get_size()-1]).key_off;
...
private:
  void inner_insert(
    ...
    if (VALIDATE_INVARIANTS) {
      if (iter != iter_begin()) {
	      assert(compare_string((iter - 1)->get_node_val(), val) == ordering_t::less);
      }
      if (iter != iter_end()) {
	      assert(compare_string(iter->get_node_val(), val) == ordering_t::greater);
      }
  ...
  void inner_replace(
    ...
    if (VALIDATE_INVARIANTS) {
      ...
      if ((iter + 1) != iter_end()) {
	      assert(compare_string((iter + 1)->get_node_val(), val) == ordering_t::greater);
      }
      assert(is_overflow(val.size() + 1) == false);
    }
    inner_remove(iter);
   ...
   */
  KINT *get_node_key_ptr() {
    return L::Partial(1, 1, get_size()).template Pointer<2>(buf);
  }
  const KINT *get_node_key_ptr() const {
    return L::Partial(1, 1, get_size()).template Pointer<2>(buf);
  }
...
private:
  void leaf_insert(
    ...
    if (VALIDATE_INVARIANTS) {
      if (iter != iter_begin()) {
	      assert(compare_string((iter - 1)->get_node_val(), key) == ordering_t::less);
      }
      if (iter != iter_end()) {
	      assert(compare_string(iter->get_node_val(), key) == ordering_t::greater);
      }
