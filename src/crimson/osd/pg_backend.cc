  ...
  object_stat_sum_t& delta_stats) const
{
  ...
  omap_iterate_cb_t callback = [&result, &num, &truncated, max_return, &start_from]
  ...
  object_stat_sum_t& delta_stats) const
{
  ...
  omap_iterate_cb_t callback = [filter_prefix, max_return, &result, &num, &truncated, &start_from]
    (std::string_view key, std::string_view value) {
      if (num >= max_return) {
        truncated = true;
      }
