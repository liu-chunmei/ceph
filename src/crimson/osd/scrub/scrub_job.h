...
class ScrubJob {
 ...
 /**
  ...
  void on_periods_change(
      ...
      const crimson::osd::scrub::sched_conf_t& aconf,
      utime_t scrub_clock_now) {}
  ...
  double guaranteed_offset(
      ...
      const crimson::osd::scrub::sched_conf_t& app_conf);
...
template <>
struct formatter<crimson::osd::scrub::SchedTarget>
{
  ...
  auto format(const crimson::osd::scrub::SchedTarget& t,
              FormatContext& ctx) const
  {
    return format_to(
      ctx.out(),
      "{{pg={}, shed_info={}, level={}, urgency={}, queued={}, high_prio={}, delayed={}}}",
      t.queued_element().pgid,
      t.queued_element().schedule,
      t.is_deep() ? "deep" : "shallow",
...
template <>
struct formatter<crimson::osd::scrub::sched_conf_t>
{
  constexpr auto parse(format_parse_context& ctx)
  {
    return ctx.begin();
  ...
  template <typename FormatContext>
  auto format(const crimson::osd::scrub::sched_conf_t& c,
              FormatContext& ctx) const
  {
    return format_to(
      ctx.out(),
      "{{shallow_intv={}, deep_intv={}, rand_ratio={}, deep_rand_ratio={}, mandatory_on_invalid={}}}",
      c.shallow_interval,
      c.deep_interval,
      c.interval_randomize_ratio,
      c.deep_randomize_ratio,
      c.mandatory_on_invalid);
  }
};
template <>
struct formatter<crimson::osd::scrub::ScrubJob> {
  constexpr auto parse(format_parse_context& ctx) {
    return ctx.begin();
  }

  template <typename FormatContext>
  auto format(const crimson::osd::scrub::ScrubJob& j, FormatContext& ctx) const {
    return format_to(
        ctx.out(),
        "ScrubJob{{pgid={}, osd={}, reg={}, blocked={}, "
        "last_issue={}, shallow={}, deep={}, blocked_since={}}}",
        j.pgid,
        j.whoami,
        j.registered,
        j.blocked,
