...
TEST_P(seastore_test_t, rename)
{
  run_async([this] {
    ...
    std::map<string, bufferlist>::iterator refiter;
    std::function<ObjectStore::omap_iter_ret_t(std::string_view, std::string_view)> callback =
      [&test_other, &start_from, &refiter](std::string_view key, std::string_view val) {
        return test_other.check_iterate(key, val, start_from, refiter);
    };
    test_obj.rename(*sharded_seastore, test_other);
    test_other.read(*sharded_seastore, 0, 4096);
    test_other.check_omap(*sharded_seastore, start_from, refiter, callback);
  });
}
...
TEST_P(seastore_test_t, omap_test_iterator)
{
  run_async([this] {
    ...
    std::map<string, bufferlist>::iterator refiter;
    std::function<ObjectStore::omap_iter_ret_t(std::string_view, std::string_view)> callback =
      [&test_obj, &start_from, &refiter](std::string_view key, std::string_view val) {
        return test_obj.check_iterate(key, val, start_from, refiter);
    };
    test_obj.check_omap(*sharded_seastore, start_from, refiter, callback);
  });
}
